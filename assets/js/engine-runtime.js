...
class BabylonSceneManager {
            ...
            initJoystick() {
                const joystickZone = this._getElementById('joystick-zone');
                ...
                if (joystickZone && window.matchMedia("(max-width: 768px)").matches) {
                    const jumpButtonContainer = this._getElementById("jump-button-container");
...
class CreativeEnginePlayer extends HTMLElement {
    ...
    async loadFromUrl(url) {
        ...
        try {
            ...
            const dataEl = doc.getElementById("workspace-data");
            if (dataEl && dataEl.textContent.trim()) {
                const b64 = dataEl.textContent.trim();
                const decodedData = decodeURIComponent(escape(atob(b64)));
...
customElements.define("creative-engine-player", CreativeEnginePlayer);
